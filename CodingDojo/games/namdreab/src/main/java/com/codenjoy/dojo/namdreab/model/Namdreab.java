...
import static com.codenjoy.dojo.namdreab.services.Event.Type.ACORN;
import static com.codenjoy.dojo.namdreab.services.Event.Type.BLUEBERRY;
import static com.codenjoy.dojo.namdreab.services.Event.Type.DEATH_CAP;
import static com.codenjoy.dojo.namdreab.services.Event.Type.EAT;
import static com.codenjoy.dojo.namdreab.services.Event.Type.FLY_AGARIC;
import static com.codenjoy.dojo.namdreab.services.Event.Type.START;
import static com.codenjoy.dojo.namdreab.services.Event.Type.STRAWBERRY;
import static com.codenjoy.dojo.namdreab.services.Event.Type.WIN;
import static com.codenjoy.dojo.namdreab.services.GameSettings.Keys.ACORNS_COUNT;
import static com.codenjoy.dojo.namdreab.services.GameSettings.Keys.BLUEBERRIES_COUNT;
import static com.codenjoy.dojo.namdreab.services.GameSettings.Keys.DEATH_CAPS_COUNT;
import static com.codenjoy.dojo.namdreab.services.GameSettings.Keys.FLY_AGARICS_COUNT;
import static com.codenjoy.dojo.namdreab.services.GameSettings.Keys.STRAWBERRIES_COUNT;
import static com.codenjoy.dojo.services.Direction.LEFT;
import static com.codenjoy.dojo.services.field.Generator.generate;
import static java.util.function.Predicate.not;
import static java.util.stream.Collectors.toList;

import com.codenjoy.dojo.namdreab.model.items.Acorn;
import com.codenjoy.dojo.namdreab.model.items.Blueberry;
import com.codenjoy.dojo.namdreab.model.items.DeathCap;
import com.codenjoy.dojo.namdreab.model.items.FlyAgaric;
import com.codenjoy.dojo.namdreab.model.items.Rock;
import com.codenjoy.dojo.namdreab.model.items.StartSpot;
import com.codenjoy.dojo.namdreab.model.items.Strawberry;
...
public class Namdreab extends RoundField<Player, Hero> implements Field {
  ...
  public Optional<Point> freeRandom(Player player) {
    ...
    TreeSet<StartSpot> startSpots = starts().stream()
        .filter(s -> freeOfHero(s))
        .collect(
            Collectors.toCollection(() -> new TreeSet<>((o1, o2) -> {
              int compare = Integer.compare(
                  distanceToClosestHero(o1),
                  distanceToClosestHero(o2)
              );
              if (compare == 0) {
                return 1;
              }
    ...
    return Optional.of(startSpots.first());
  }

  private int distanceToClosestHero(Point point) {
    int min = Integer.MAX_VALUE;
    for (Hero hero : heroes()) {
      if (hero != null) {
        Optional<Integer> minDistance = hero.body().stream()
            .map((o) -> Math.abs(o.getX() - point.getX()) + Math.abs(o.getY() - point.getY()))
            .sorted()
            .findFirst();
